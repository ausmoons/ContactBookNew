  ...
  <div class="contacts container">
    ...
    <div v-for="contact in filterBy(contacts, filterInput)" :key="contact.id">
      <md-card md-with-hover>
        ...
        <md-card-actions>
          <md-button>
            <router-link class="btn btn-default" v-bind:to="'/contactDetails/id/'+contact.id">View</router-link>
          </md-button>
        </md-card-actions>
      </md-card>
    </div>
    ...
    <div class="container">
      <div id="navbar" class="collapse navbar-collapse">
        <ul class="nav navbar-nav navbar-right">
          <md-button>
            <router-link class="btn btn-default" to="/add">Add Contact</router-link>
          </md-button>
        </ul>
      </div>
      <!--/.nav-collapse -->
...
export default {
  ...
  methods: {
    fetchContacts() {
      this.axios
        .get("https://localhost:44366/api/Contact/All")
        .then(({ data }) => {
